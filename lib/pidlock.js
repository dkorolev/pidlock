...
module.exports = function(dir, key, callback) {
    ...
    createExitHook = function(data) {
        return function() {
            assert(_.isObject(data));
            assert(_.isString(data.me_dir));
            assert(_.isString(data.lock_dir));
    ...
    try {
        ...
        try {
            ...
            data.state = 'OK';
            callback(null, data, createExitHook(data));
        } catch (e) {
...
if (require.main === module) {
    ...
    (function speakUp() {
        setTimeout(function() {
            module.exports('/tmp', 'pidlock_test', function(error, data, cleanup) {
                if (error) {
                    speakUp();
                } else {
